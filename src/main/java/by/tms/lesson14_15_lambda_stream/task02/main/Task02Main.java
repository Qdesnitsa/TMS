package by.tms.lesson14_15_lambda_stream.task02.main;

import static java.util.Arrays.asList;

import by.tms.lesson14_15_lambda_stream.task02.entity.Gender;
import by.tms.lesson14_15_lambda_stream.task02.entity.Hobby;
import by.tms.lesson14_15_lambda_stream.task02.entity.People;
...
public class Task02Main {
  ...
  public static void main(String[] args) {
    ...
    obtainMapNameGender(peopleList);

    // 10. Решение
    System.out.println("Task10: ");
    obtainMapScoreSumDesc(peopleList);
  }
  ...
  public static List<People> peopleEachGenderAgeRange(List<People> list, int femaleStartAge,
    ...
    Stream<People> resultStream = Stream.concat(
        ...
        list.stream().flatMap(s -> Stream.ofNullable(s))
            .filter(female.and(startAge1.and(endAge1))));
  ...
  public static List<People> sortGenderAgeAsc(List<People> list) {
    List<People> targetList = list.stream()
        .flatMap(s -> Stream.ofNullable(s))
        .sorted(
            (p1, p2) -> p1.getGender().equals(p2.getGender()) ? p1.getAge().compareTo(p2.getAge())
                : p1.getGender().compareTo(p2.getGender()))
        .peek(System.out::println)
        .collect(Collectors.toList());
    return targetList;
  }
  ...
  public static List<People> obtainListScoreHobby(List<People> list, Gender gender1, Double score1,
      int hobby1, Gender gender2, Double score2, int hobby2) {
    List<People> targetList = list.stream()
        .flatMap(s -> Stream.ofNullable(s))
        .filter(p ->
            p.getGender().equals(gender1) &&
                p.getSizeList() >= hobby1 &&
                p.getListHobby().stream().flatMap(s -> Stream.ofNullable(s))
                    .mapToInt(Hobby::getScore).average().getAsDouble() > score1 ||
                p.getGender().equals(gender2) &&
                    p.getSizeList() >= hobby2 &&
                    p.getListHobby().stream().flatMap(s -> Stream.ofNullable(s))
                        .mapToInt(Hobby::getScore).average().getAsDouble() > score2)
        .peek(System.out::println)
        .collect(Collectors.toList());
    return targetList;
  }

  // 9. Вывести мар - пол и люди.
  public static void obtainMapNameGender(List<People> list) {
   list.stream()
        .flatMap(s -> Stream.ofNullable(s))
        .collect(
            Collectors.groupingBy(People::getGender,Collectors.mapping(People::getName,
                    Collectors.joining(", "))))
                .forEach((k,v) -> System.out.printf("%s=%s\n",k,v));
  }

  // 10. Вывести мар - человек и сумма баллов по хобби с убыванием по баллам.
  public static void obtainMapScoreSumDesc(List<People> list) {
   list.stream()
        .flatMap(s -> Stream.ofNullable(s))
        .collect(
            Collectors.groupingBy(People::getName, Collectors.summingInt(p -> p.getListHobby()
                    .stream()
                    .flatMap(f -> Stream.ofNullable(f))
                    .mapToInt(Hobby::getScore)
                    .sum())))
       .entrySet().stream()
